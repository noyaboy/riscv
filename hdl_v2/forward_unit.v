...
module forward_unit (
    ...
    assign rs1_ID_fwd = (((op_EX==`R_TYPE || op_EX==`I_TYPE_OP_IMM) && (op_ID==`R_TYPE || op_ID==`I_TYPE_LOAD || op_ID==`I_TYPE_OP_IMM || op_ID==`S_TYPE) && rd_waddr_EX == rs1_raddr_ID && ~(rs1_raddr_ID == 0))==1)? 1:
        (((op_M==`R_TYPE || op_M==`I_TYPE_OP_IMM) && op_ID==`B_TYPE && rd_waddr_M == rs1_raddr_ID)==1 && ~(rs1_raddr_ID == 0))? 2:
        ((op_M==`I_TYPE_LOAD && (op_ID==`R_TYPE || op_ID==`I_TYPE_LOAD || op_ID==`I_TYPE_OP_IMM || op_ID==`S_TYPE) && rd_waddr_M == rs1_raddr_ID && ~(rs1_raddr_ID == 0))==1)? 3: 
        (((op_RB==`I_TYPE_LOAD && op_ID==`B_TYPE) && rd_waddr_RB == rs1_raddr_ID && ~(rs1_raddr_ID == 0))==1) ? 4: 0;

    assign rs2_ID_fwd = ((op_EX==`R_TYPE && op_ID==`R_TYPE && rd_waddr_EX == rs2_raddr_ID && ~(rs2_raddr_ID == 0))==1)? 1:
    ...
    assign rs2_EX_fwd = ((op_M==`R_TYPE && op_EX==`S_TYPE && rd_waddr_M == rs2_raddr_EX && ~(rs2_raddr_EX == 0))==1)? 1:
